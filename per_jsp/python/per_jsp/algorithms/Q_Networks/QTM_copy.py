...
class QTM:
    def __init__(self, env, Policy, config):
        ...
        sample_size = config.get("sample_size", config.get("batch_size", 32))
        n_steps = config.get("n_steps", 1)
        self.replay_buffer = ReplayBuffer(config["buffer_size"], sample_size, n=n_steps)
    ...
    def rollout(self):
        ...
        while True:
            ...
            sample_size = self.config.get("sample_size", self.config.get("batch_size", 32))
            n_steps = self.config.get("n_steps", 1)
            train_freq = self.config.get("train_freq", 1)
            
            if self.total_timesteps >= sample_size + n_steps and self.total_timesteps % train_freq == 0:
                if n_steps > 1:
                    self.train_n_step()
                else:
                    self.train()
