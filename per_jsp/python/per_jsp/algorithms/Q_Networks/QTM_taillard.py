...
from per_jsp.python.per_jsp.algorithms.misc.replay_buffer import ReplayBuffer
from  per_jsp.python.per_jsp.algorithms.Q_Networks.JSPBinarize_taillard import JSPBinarizer  # Import the JSPBinarizer
...
class QTM:
    ...
    """
    def __init__(self, env, Policy, config, num_jobs=None, num_machines=None, max_operations=None, feature_bits=8):
        ...
        # Initialize the JSPBinarizer
        feature_bits = config.get("feature_bits", 8)
        self.binarizer = JSPBinarizer(env, 
                                      feature_bits=feature_bits, 
                                      num_jobs=num_jobs, 
                                      num_machines=num_machines,
                                      max_operations=max_operations)
        
        # Add binarizer to the policy config
        config['binarizer'] = self.binarizer
        config['num_features'] = self.binarizer.feature_count
        
        # Get binary observation size instead of flattened size
        test_obs, _ = env.reset()
        binary_obs = self.binarizer.transform(test_obs)
        self.num_features = binary_obs.size
        
        config['action_space_size'] = self.action_space_size
        config['obs_space_size'] = config['num_features'] = self.num_features
        config['binarizer'] = self.binarizer

        self.online_policy = Policy(env,config)
    ...
    def announce(self):
        print(f'JSP QTM {self.run_id} with JSPBinarizer has been initialized!')
    ...
    def get_next_action(self, cur_obs):
        ...
        """
        # Use binarizer to transform the observation
        binary_obs = self.binarizer.transform(cur_obs)
        ...
        if np.random.random() < self.epsilon:
            ...
            q_vals = self.online_policy.predict(binary_obs)[0]
    ...
    def rollout(self):
        ...
        while not (terminated or truncated):
            # Convert observation to binary using binarizer
            ...
            # Transform observations using binarizer
            binary_obs = self.binarizer.transform(obs)
            binary_next_obs = self.binarizer.transform(next_obs)
            
            # Store experience in replay buffer using binary observations
            self.replay_buffer.save_experience(
                action, binary_obs, binary_next_obs, reward, int(terminated), truncated
            )
    ...
    def test(self, nr_of_steps):
        """Test the current policy on Job Shop Scheduling problems"""
        ...
        for episode in range(self.nr_of_test_episodes):
            ...
            while not (terminated or truncated):
                ...
                if len(valid_jobs) == 0:
                    action = 0  # No valid jobs, use default
                else:
                    # Get Q-values and mask invalid actions
                    q_vals = self.online_policy.predict(obs)
                    masked_q_vals = np.ones(self.action_space_size) * float('-inf')
                    masked_q_vals[valid_jobs] = q_vals[0][valid_jobs]
            ...
            if 'makespan' in info:
                ...
                episode_makespans[episode] = step_count
        
        # Calculate statistics
        valid_makespans = episode_makespans[episode_makespans > 0]
        if len(valid_makespans) > 0:
            mean_makespan = np.mean(valid_makespans)
            std_makespan = np.std(valid_makespans)
        else:
            mean_makespan = 1000
            std_makespan = 0
