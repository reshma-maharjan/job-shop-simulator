...
#from per_jsp.python.per_jsp.algorithms.ppo_do import PPOScheduler
from per_jsp.python.per_jsp.algorithms.ppo_2 import PPOScheduler
from per_jsp.python.per_jsp.algorithms.dqn import DQNScheduler

from per_jsp.python.per_jsp.algorithms.qtm import HybridTsetlinQLearningScheduler
from per_jsp.python.per_jsp.algorithms.double_qtm import DoubleQTsetlinScheduler 
...
def get_performance_metrics(self) -> dict:
    ...
    # Calculate average waiting time if there are any actions
    if self.action_history:
        ...
        metrics["avg_waiting_time"] = float(np.mean(waiting_times))
        metrics["max_waiting_time"] = float(np.max(waiting_times))
    else:
...
def create_scheduler(args) -> BaseScheduler:
    ...
    elif args.algorithm == "hybrid-tm-q":
        return HybridTsetlinQLearningScheduler(
            learning_rate=args.learning_rate,
            discount_factor=args.discount_factor,
            exploration_rate=args.epsilon,
            episodes=args.episodes,
            nr_clauses=args.tm_clauses,
            T=args.tm_threshold,
            s=args.tm_specificity,
            optimal_makespan=args.optimal_makespan
        )
    elif args.algorithm == "double-qtm":
        return DoubleQTsetlinScheduler(
            learning_rate=args.learning_rate,
            discount_factor=args.discount_factor,
            exploration_rate=args.epsilon,
            episodes=args.episodes,
...
def setup_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--algorithm", type=str, 
                   choices=["greedy", "q-learning", "ppo", "dqn", "hybrid-tm-q", "double-qtm"],
    ...
    # Hybrid Tsetlin Machine-Q Learning parameters/ Double-Q Learning Tsetlin Machine
    parser.add_argument("--tm-clauses", type=int, default=10,
                       help="Number of clauses for Tsetlin Machine")
    parser.add_argument("--tm-threshold", type=float, default=1500,
                       help="Threshold (T) parameter for Tsetlin Machine")
    parser.add_argument("--tm-specificity", type=float, default=1.5,
                       help="Specificity (s) parameter for Tsetlin Machine")
...
def main() -> int:
    ...
    try:
        ...
        # add another parameter to the args for optimal makespan
        args.optimal_makespan = estimated_makespan
        ...
        elif args.algorithm in ["hybrid-tm-q", "double-qtm"]: 
            algorithm_params.update({
                "learning_rate": args.learning_rate,
                "discount_factor": args.discount_factor,
                "epsilon": args.epsilon,
                "episodes": args.episodes,
                "tm_clauses": args.tm_clauses,
                "tm_threshold": args.tm_threshold,
                "tm_specificity": args.tm_specificity
            })
