...
from per_jsp.python.per_jsp.algorithms.tm_jssp import TsetlinScheduler
...
def create_scheduler(args) -> BaseScheduler:
    ...
    elif args.algorithm == "tsetlin":
        return TsetlinScheduler(
            episodes=args.episodes,
            nr_clauses=args.clauses,
            T=args.threshold,
            s=args.specificity,
...
def setup_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--algorithm", type=str, choices=["greedy", "tsetlin"],
                       default="tsetlin", help="Algorithm to use")
...
def main() -> int:
    ...
    try:
        ...
        logger.info(f"Environment: {len(jobs)} jobs, {env.num_machines} machines")
        ...
        logger.info(f"Avg machine utilization: {metrics['avg_machine_utilization']*100:.1f}%")
